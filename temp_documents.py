...
def remove_temp_document(doc_name: str, embeddings):
    ...
    # Retirer le document
    st.session_state.temp_documents = [
        d for d in st.session_state.temp_documents 
        if d["name"] != doc_name
    ]
...
def render_temp_documents_section(embeddings):
    ...
    if uploaded_files:
        with st.spinner("📚 Chargement des documents..."):
            ...
            if new_count > 0:
                st.success(f"✅ {new_count} document(s) ajouté(s) ({total_chunks} chunks)")
    ...
    if info["count"] > 0:
        ...
        st.caption(f"Total: {info['total_chunks']} chunks • {format_file_size(info['total_size'])}")
        ...
        for i, doc in enumerate(info["documents"]):
            ...
            with col1:
                ...
                st.caption(f"   {doc['chunks']} chunks • {format_file_size(doc['size'])}")
            with col2:
