...
def test_rag_query_logs_correctly(mock_retriever, temp_logs_db, monkeypatch):
    """Test que rag_query retourne les bonnes infos pour le logging."""
    import config
    from rag_pipeline import rag_query
    ...
    with patch("rag_pipeline.PromptTemplate.from_template"):
        with patch("rag_pipeline.StrOutputParser"):
            # Forcer une erreur pour tester la gestion d'erreur
            mock_retriever.invoke.side_effect = Exception("Test error")

            result = rag_query("test query", mock_retriever, mock_llm, logger=None)

            # ✅ MODIFIÉ : Vérifier que le résultat contient l'erreur
            assert result["error"] == "Test error"
            assert result["answer"] is None
            assert result["sources"] == []

            # ✅ NOUVEAU : Logger manuellement comme le fait app_chat.py
